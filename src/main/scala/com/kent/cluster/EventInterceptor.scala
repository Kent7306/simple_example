...
class EventInterceptor extends ClusterRoledWorker {
  ...
  override def receive: Actor.Receive = {
    case MemberUp(member) =>
      log.info("*********************Member is Up: {}, role is: {}", member.address, member.getRoles)
      if(member.getRoles.contains("collector")){
    ...
    case NginxRecord(sourceHost, eventCode, line) => {
      ...
      if(!isIpInBlackList) {
        ...
        if(workers.size > 0) {
          ...
          workers(processorIndex) ! FilteredRecord(sourceHost, eventCode, line, data("eventdate"), data("realip"))
