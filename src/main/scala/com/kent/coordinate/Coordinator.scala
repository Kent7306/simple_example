...
class Coordinator(val name: String) {
  ...
  def execute(wfManager: ActorRef): Boolean = {
    ...
    if(isSatisfyTrigger()) {
      println(s"[coordinator:${this.name}]开始触发[workflow:${this.workflow}]")
      this._status = ACTIVE
      wfManager ! NewAndExecuteWorkFlowInstance(this.workflow,this.paramMap)
      this.resetTrigger()
      true
    } else {
      false
...
object Coordinator {
  ...
  def pareseXmlNode(node: scala.xml.Node): Coordinator = {
    ...
    val workFlowName = (node \ "workflow" \ "@path").text
